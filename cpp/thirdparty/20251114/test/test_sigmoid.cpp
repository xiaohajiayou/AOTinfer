...
int main(gkcl_s32 argc, char *argv[])
{
    ...
    */
    ...
    ret = gkcl_op_sigmoid_get_workspace_size(tensor, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_sigmoid_get_workspace_size err, errno %d\n", ret);
    }
    if (workspace_size > 0) {
        workspace = malloc(workspace_size);
    }
    ret = gkcl_op_sigmoid(workspace, workspace_size, executor, NULL);
    executor = NULL;
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_sigmoid err, errno %d\n", ret);
    }
    executor = NULL;
    if (workspace_size > 0) {
        free(workspace);
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "sigmoid_op");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "sigmoid_op");
        if (ret != GKCL_SUCCESS) {
