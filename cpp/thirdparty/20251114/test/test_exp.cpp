...
int main()
{
    ...
    gkcl_layout input_layout =
        GKCL_LAYOUT_BM1N1M0N0; // GKCL_LAYOUT_BMN GKCL_LAYOUT_BM1N1M0N0 /GKCL_LAYOUT_NCHW GKCL_LAYOUT_NC1HWC0
    gkcl_layout output_layout = GKCL_LAYOUT_BM1N1M0N0;
    ...
    */
    ...
    ret = gkcl_op_exp_get_workspace_size(tensor, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_exp_get_workspace_size err, errno %d\n", ret);
    }
    if (workspace_size > 0) {
        workspace = malloc(workspace_size);
    }
    ret = gkcl_op_exp(workspace, workspace_size, executor, NULL);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_exp err, errno %d\n", ret);
    }
    executor = NULL;
    if (workspace_size > 0) {
        free(workspace);
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "exp_op");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "exp_op");
        if (ret != GKCL_SUCCESS) {
