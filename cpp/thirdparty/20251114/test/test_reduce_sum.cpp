...
int main(gkcl_s32 argc, char *argv[])
{
    ...
    xmnpu_uint8 temp_val = 0;
    gkcl_data_type dtype_str[3] = {GKCL_FP16, GKCL_FP32, GKCL_BF16};

    temp_val = rand() % 3;
    gkcl_data_type input_dtype = dtype_str[temp_val];
    gkcl_data_type output_dtype;
    if (input_dtype == GKCL_FP16) {
        output_dtype = (rand() % 2) ? GKCL_FP16 : GKCL_FP32;
    } else if (input_dtype == GKCL_BF16) {
        output_dtype = (rand() % 2) ? GKCL_BF16 : GKCL_FP32;
    } else {
        assert(input_dtype == GKCL_FP32);
        output_dtype = GKCL_FP32;
    }
    ...
    gkcl_op_reduce_sum_param param;
    ...
    std::vector<uint32_t> output_dims = {param.axes == 0 ? 1 : batch, param.axes == 1 ? 1 : m, param.axes == 2 ? 1 : n};
    // batch = 3;
    // m = 5;
    // n = 10;
    ...
    if (output_layout == GKCL_LAYOUT_BM1N1M0N0) {
        ...
        output_byte_num = batch * m * XMNPU_ALIGN_FUNC(gkcl_u64(output_byte * n), 128);
    }
    ...
    ret = gkcl_op_reduce_sum_get_workspace_size(tensor, &param, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_reduce_sum_get_workspace_size err, errno %d\n", ret);
    }

    ret = gkcl_op_reduce_sum(NULL, 0, executor, NULL);
    executor = NULL;
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_softmax err, errno %d\n", ret);
    }
    executor = NULL;
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "reduce_sum");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "reduce_sum");
        if (ret != GKCL_SUCCESS) {
