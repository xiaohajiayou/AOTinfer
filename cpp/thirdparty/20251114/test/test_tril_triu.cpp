...
int main()
{
    ...
    if (input_layout == GKCL_LAYOUT_BM1N1M0N0) {
        ...
        input_byte_num = batch * m * XMNPU_ALIGN_FUNC(gkcl_u64(input_byte * n), 1);
    }
    ...
    if (output_layout == GKCL_LAYOUT_BM1N1M0N0) {
        ...
        output_byte_num = batch * m * XMNPU_ALIGN_FUNC(gkcl_u64(output_byte * n), 1);
    }
    ...
    gkcl_op_tril_triu_param param;
    param.diagonal = 0;
    param.mask_value = 0;

    int tril_triu_mode = rand() % 2;

    uint64_t workspace_size = 0;
    void *workspace = NULL;
    gkcl_op_executor executor;
    if (tril_triu_mode == 0) {
        ret = gkcl_op_tril_get_workspace_size(tensor, &param, tensor_out, &workspace_size, &executor);
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_sin_get_workspace_size err, errno %d\n", ret);
        }
        if (workspace_size > 0) {
            workspace = malloc(workspace_size);
        }
        ret = gkcl_op_tril(workspace, workspace_size, executor, NULL);
        executor = NULL;
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_sin err, errno %d\n", ret);
        }
    } else {
        ret = gkcl_op_triu_get_workspace_size(tensor, &param, tensor_out, &workspace_size, &executor);
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_sin_get_workspace_size err, errno %d\n", ret);
        }
        if (workspace_size > 0) {
        ...
        ret = gkcl_op_triu(workspace, workspace_size, executor, NULL);
        executor = NULL;
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_sin err, errno %d\n", ret);
        }
    ...
    for (int i = 0; i < output_size; i++) {
        if (i < 10) {
            ...
            if (output_dtype == GKCL_BF16) {
                printf("%.3f ", XMNPU_ASFLOAT(xmnpu_cmodel_bf16_to_f32(output_addr[i])));
            } else if (output_dtype == GKCL_FP32) {
                printf("%.3f ", XMNPU_ASFLOAT(output_addr_32[i]));
            }
        } else {
            break;
        }
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "sin_op");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "sin_op");
        if (ret != GKCL_SUCCESS) {
