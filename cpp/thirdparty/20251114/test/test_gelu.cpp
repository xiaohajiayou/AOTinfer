...
int main()
{
    ...
    */
    ...
    ret = gkcl_op_gelu_get_workspace_size(tensor, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
    ...
    ret = gkcl_op_gelu(workspace, workspace_size, executor, NULL);
    executor = NULL;
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_matmul err, errno %d\n", ret);
    }
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "gelu");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "gelu");
        if (ret != GKCL_SUCCESS) {
