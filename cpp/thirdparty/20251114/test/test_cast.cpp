...
int main()
{
    ...
    std::vector<uint32_t> input_dims = {2, 10, 128};
    uint64_t *input_pitch = (uint64_t *)malloc(input_dims.size() * sizeof(uint64_t));
    gkcl_get_tensor_pitch(GKCL_FP16, input_dims.size(), input_dims.data(), input_pitch, GKCL_LAYOUT_OTHERS,
                          GKCL_TENSOR_INPUT);
    for (size_t i = 0; i < input_dims.size(); i++) {
        printf("input pitch[%d] :%lld\n", i, input_pitch[i]);
    }
    gkcl_u8 *input = (gkcl_u8 *)malloc(input_dims[0] * input_pitch[0]);
    gkcl_s32 input_size = std::accumulate(input_dims.begin(), input_dims.end(), 1, std::multiplies<int>());
    gkcl_s32 input_num = input_size / 2;
    ...
    std::vector<uint32_t> output_dims = {2, 10, 128};
    uint64_t *output_pitch = (uint64_t *)malloc(output_dims.size() * sizeof(uint64_t));
    gkcl_get_tensor_pitch(GKCL_UINT8, output_dims.size(), output_dims.data(), output_pitch, GKCL_LAYOUT_OTHERS,
                          GKCL_TENSOR_OUTPUT);
    for (size_t i = 0; i < output_dims.size(); i++) {
        printf("output pitch[%d] :%lld\n", i, output_pitch[i]);
    }
    int output_size = output_dims[0] * output_pitch[0];
    // gkcl_u8 *output = (gkcl_u8 *)malloc(output_dims[0] * output_pitch[0]);
    // memset(output, 0, output_size);
    ret = gkcl_create_tensor(&tensor, GKCL_FP16, 3, input_dims.data(), input_pitch, GKCL_LAYOUT_OTHERS, input,
                             GKCL_TENSOR_INPUT);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_create_tensor errno %d\n", ret);
    }
    ret = gkcl_create_tensor(&tensor_out, GKCL_UINT8, 3, output_dims.data(), output_pitch, GKCL_LAYOUT_OTHERS, input,
                             GKCL_TENSOR_OUTPUT);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_create_tensor errno %d\n", ret);
    }
    ...
    ret = gkcl_op_cast_get_workspace_size(tensor, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_permute err, errno %d\n", ret);
    }
    ret = gkcl_op_cast(NULL, 0, executor, NULL);
    executor = NULL;
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_permute err, errno %d\n", ret);
    }
    executor = NULL;
    gkcl_u8 *output_addr = (gkcl_u8 *)input;
    printf("output value : ");
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "cast_op");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "cast_op");
        if (ret != GKCL_SUCCESS) {
