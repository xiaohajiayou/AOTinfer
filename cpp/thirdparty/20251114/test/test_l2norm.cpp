...
int main(gkcl_s32 argc, char *argv[])
{
    ...
    gkcl_op_l2norm_param param;
    param.axes = orig_rank - 1;
    ...
    ret = gkcl_op_l2norm_get_workspace_size(tensor, &param, tensor_out, &workspace_size, &executor);
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_matmul err, errno %d\n", ret);
    }
    struct timeval tv2;
    gettimeofday(&tv2, NULL);
    ret = gkcl_op_l2norm(NULL, 0, executor, NULL);
    struct timeval tv3;
    gettimeofday(&tv3, NULL);
    printf("gkcl_op_l2norm cost time: %ld ms \n",
           ((tv3.tv_sec - tv2.tv_sec) * 1000 + (tv3.tv_usec - tv2.tv_usec) / 1000));
    executor = NULL;
    if (ret != GKCL_SUCCESS) {
        printf("gkcl_op_l2norm err, errno %d\n", ret);
    }
    executor = NULL;
    ...
    if (run_mode == GKCL_OP_RUN_MODE_SERI) {
        ret = gkcl_op_serialize_model(context, "l2norm");
        if (ret != GKCL_SUCCESS) {
            printf("gkcl_op_serialize_model errno: %d\n", ret);
        }
        ret = test_inference_code_data_model(context, "l2norm");
        if (ret != GKCL_SUCCESS) {
