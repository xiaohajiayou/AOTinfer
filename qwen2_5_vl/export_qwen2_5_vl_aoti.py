...
This variant assumes vision features are already projected to hidden size.
During export we use random vision tokens; at runtime pass real tokens or a zero-length tensor.
"""


def flatten_inputs(input_ids, vision_tokens, key_cache, value_cache, cache_len):
    flat = [input_ids, vision_tokens]
...
class Qwen2_5_VLAOTIWrapper(torch.nn.Module):
    def __init__(self, export_model):
        super().__init__()
        self.export_model = export_model
        self.num_layers = len(export_model.layers)
        self.hidden_size = export_model.config.hidden_size

    def forward(self, *flat_inputs):
        input_ids = flat_inputs[0]
        vision_tokens = flat_inputs[1]
        cache_inputs = flat_inputs[2:]
        ...
        if vision_tokens is None:
            vision_tokens = torch.zeros(
                ...
                dtype=self.export_model.embed.weight.dtype,
            )
...
def export_and_package(args):
    ...
    # use random vision tokens; expected to be projected to hidden_size already
    ...
    N_dim = torch.export.Dim("N_text", min=1, max=adapter.config.max_position_embeddings)
    V_dim = torch.export.Dim("N_vision", min=0, max=adapter.config.max_position_embeddings)
    cache_dim = torch.export.Dim("cache_len", min=0, max=adapter.config.max_position_embeddings)
    per_input_shapes = []
    per_input_shapes.append({1: N_dim})  # input_ids
    per_input_shapes.append({1: V_dim})  # vision_tokens
...
def verify_with_aoti(args, adapter, wrapper, package_path):
    ...
    def run_wrapper(input_ids, vision_tokens, key_cache, value_cache, cache_len):
        if vision_tokens is None:
            vision_tokens = torch.zeros(
                input_ids.shape[0],
                0,
        ...
        out = wrapper(*flatten_inputs(input_ids, vision_tokens, key_cache, value_cache, cache_len))
        return unflatten_outputs(out, num_layers)

    def run_loader(input_ids, vision_tokens, key_cache, value_cache, cache_len):
        if vision_tokens is None:
            vision_tokens = torch.zeros(
                input_ids.shape[0],
                0,
        ...
        out = loader.run(flatten_inputs(input_ids, vision_tokens, key_cache, value_cache, cache_len))
        return unflatten_outputs(out, num_layers)

    prompt = args.prompt
    tok = tokenizer(prompt, return_tensors="pt").to(args.device)
    # random vision tokens; replace with real projected features if available
    vision_tokens = torch.randn(
        1,
        args.example_vision_tokens,
        adapter.config.hidden_size,
        device=args.device,
        dtype=dtype,
    )
    ...
    logits_w, key_cache_w, value_cache_w, cache_len_w = run_wrapper(
        tok["input_ids"], vision_tokens, key_cache_w, value_cache_w, cache_len_w
    )
    logits_l, key_cache_l, value_cache_l, cache_len_l = run_loader(
        tok["input_ids"], vision_tokens, key_cache_l, value_cache_l, cache_len_l
    )
    ...
    for step in range(args.decode_steps):
        logits_w, key_cache_w, value_cache_w, cache_len_w = run_wrapper(
            next_token, None, key_cache_w, value_cache_w, cache_len_w
        )
        logits_l, key_cache_l, value_cache_l, cache_len_l = run_loader(
            next_token, None, key_cache_l, value_cache_l, cache_len_l
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export Qwen2.5-VL export model to AOTI package and verify")
    parser.add_argument("-m", "--model_path", type=str, required=True)
    parser.add_argument("-o", "--out_dir", type=str, required=True)
    parser.add_argument("--package_name", type=str, default="qwen2_5_vl_export.pt2")
    ...
    parser.add_argument("--example_vision_tokens", type=int, default=64, help="example vision token count for export")
...
# python -m qwen2_5_vl.export_qwen2_5_vl_aoti \
#   -m /Users/bruceli/Desktop/Git_sync/model/Qwen2.5-VL-3B \
#   -o /Users/bruceli/Desktop/Git_sync/model/pt2 \
#   --package_name qwen2.5_vl.pt2 \
#   --device cpu \
#   --torch_dtype float32 \
#   --example_vision_tokens 8 \
#   --example_seq 2 \
#   --example_cache_len 2
